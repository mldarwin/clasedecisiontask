function [subjdata] = clase_task_noconditions(subjID, do_full, do_TTL, do_prac, device)

% subjID = '992';
% do_full = 1;
% do_TTL = 1;
% do_prac = 1;
% Last update to this f(x) on this machine: 07.06.22 -MLD
%
%
% To run task: clase_task_noconditions('997', 1, 1, 1, device)
%
% CHANGED pwd on line 76
%
...
% 
%device:
%       Enables hexidecimal TTL encoding. Must run 'Cedrus_TTL' function first.
%       Must have 'get Byte' and 'setPulseDiration' functions on path. 
%
...
try
    ...
    if do_prac     
        ...
        for t = 1:nTp
            ...
            if do_TTL
                ...
                write(device,sprintf("mh%c%c",40, 0), "char"); % TTL pulse marking start of response window
            end
            ...
            if do_TTL
                ...
                write(device,sprintf("mh%c%c",15, 0), "char"); % TTL pulse marking start of ISI
            end
            ...
            if do_TTL
                ...
                write(device,sprintf("mh%c%c",50, 0), "char"); % TTL pulse marking outcome presentation
            end
            ...
            if do_TTL
                ...
                write(device,sprintf("mh%c%c",25, 0), "char"); % TTL pulse marking start of ITI
            end
    ...
    if do_TTL
        ...
        write(device,sprintf("mh%c%c",60, 0), "char"); % TTL pulse marking start of the actual study
    end
    ...
    for t = triStart:nT
        ...
        if do_TTL
            ...
            write(device,sprintf("mh%c%c", 80, 0), "char"); % TTL pulse marking start of the response window
        end
        ...
        if do_TTL
            ...
            write(device,sprintf("mh%c%c",81, 0), "char"); % TTL pulse marking end of the response window (due to button press OR expiration of time
        end
        ...
        if do_TTL
            ...
            write(device,sprintf("mh%c%c", 90, 0), "char"); % TTL pulse marking start of the outcome epoch
        end
        ...
        if do_TTL
            ...
            write(device,sprintf("mh%c%c", 91, 0), "char"); % TTL pulse marking end of outcome epoch
        end
    ...
    if do_TTL
        ...
        write(device,sprintf("mh%c%c", 61, 0), "char"); % TTL pulse marking end of the study
    end
